...
import type { CmsEntry } from "@webiny/content-sdk-nextjs";
...
export const AuthorBlockComponent = ({ author }: AuthorBlockProps) => {
    console.log(
        "[AuthorBlock] author prop:",
        author,
        "has values:",
        author ? "values" in author : false
    );

    if (!author || !("values" in author)) {
        console.log("[AuthorBlock] no values, rendering loading state");
